...
class TestEvent(unittest.TestCase):
    ...
    def test_event_types_exist(self):
        ...
        # 20 original + 11 IssueWatcher events + 3 Intent Enhancement events + 3 PRD Revision events + 2 PRD completion events = 39
        self.assertEqual(len(EventType), 39)
...
class TestEnhanceAllCLI(unittest.TestCase):
    ...
    def test_enhance_all_with_override_flags(self):
        args = self.parser.parse_args(["--enhance-all", "--no-revise-prd"])
        self.assertTrue(args.enhance_all)
        self.assertTrue(args.no_revise_prd)
...
class TestEnhanceAllCLIPassthrough(unittest.TestCase):
    ...
    def test_enhance_all_with_multiple_overrides(self):
        with patch('pyralph.RalphOrchestrator') as mock_orch:
            mock_orch.return_value = MagicMock()
            with patch('sys.argv', ['ralph', '--enhance-all', '--no-enhance-intent', '--no-revise-prd']):
                with patch('pyralph.logger.Logger.info'):
                    main()
            call_kwargs = mock_orch.call_args[1]
            self.assertFalse(call_kwargs.get('enhance_intent'))
