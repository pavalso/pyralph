...
class TestOrchestratorPromptFile(TempConfigTestCase):
    """Tests for orchestrator prompt file override."""

    def test_prompt_file_override(self):
        custom = self.temp_path / "custom.md"
        custom.write_text("Custom instructions", encoding='utf-8')
        orch = self.create_mock_orchestrator(prompt_file=str(custom))
        prd = {"id": "PRD-001", "description": "Test PRD"}
        task = {"id": "T-001", "description": "Test task"}
        result = orch._load_user_context(prd, task, "pytest")
        self.assertEqual(result, "Custom instructions")

    def test_prompt_file_not_found_exits(self):
        orch = self.create_mock_orchestrator(prompt_file="/nonexistent")
        prd = {"id": "PRD-001", "description": "Test PRD"}
        task = {"id": "T-001", "description": "Test task"}
        with patch('ralph.sys.exit', side_effect=SystemExit(1)), patch('ralph.Logger.error'):
            with self.assertRaises(SystemExit):
    ...
    def test_prompt_file_variables(self):
        custom = self.temp_path / "var.md"
        custom.write_text("PRD: {{PRD_ID}} Task: {{TASK_ID}}", encoding='utf-8')
        orch = self.create_mock_orchestrator(prompt_file=str(custom))
        prd = {"id": "PRD-001", "description": "Test PRD"}
        task = {"id": "T-002", "description": "Test task"}
        result = orch._load_user_context(prd, task, "pytest")
        self.assertIn("PRD-001", result)
        self.assertIn("T-002", result)
