...
class TestTemplateManagerDelete(TempConfigTestCase):
    """Tests for TemplateManager.delete() method."""

    def setUp(self):
        super().setUp()
        # Also configure TEMPLATES_DIR to use temp directory
    ...
    def test_delete_existing_template(self):
        """Should delete an existing template file."""
        template_path = CONF.TEMPLATES_DIR / "mytemplate.txt"
        template_path.write_text("test content", encoding="utf-8")
        self.assertTrue(template_path.exists())

        TemplateManager.delete("mytemplate.txt")

        self.assertFalse(template_path.exists())

    def test_delete_template_without_extension(self):
        """Should delete template when name provided without extension."""
        template_path = CONF.TEMPLATES_DIR / "mytemplate.txt"
        template_path.write_text("test content", encoding="utf-8")
        self.assertTrue(template_path.exists())

        TemplateManager.delete("mytemplate")

        self.assertFalse(template_path.exists())

    def test_delete_template_with_extension_exact_match(self):
        """Should use exact match when extension is provided."""
        template_path = CONF.TEMPLATES_DIR / "mytemplate.md"
        template_path.write_text("test content", encoding="utf-8")
        self.assertTrue(template_path.exists())

        TemplateManager.delete("mytemplate.md")
    ...
    def test_delete_template_not_found(self):
        ...
        with self.assertRaises(FileNotFoundError) as ctx:
            TemplateManager.delete("nonexistent")
        self.assertIn("Template nonexistent not found", str(ctx.exception))

    def test_delete_template_empty_name(self):
        """Should raise ValueError for empty template name."""
        with self.assertRaises(ValueError) as ctx:
            TemplateManager.delete("")
        self.assertIn("Invalid template name", str(ctx.exception))

    def test_delete_template_whitespace_name(self):
        """Should raise ValueError for whitespace-only template name."""
        with self.assertRaises(ValueError) as ctx:
            TemplateManager.delete("   ")
        self.assertIn("Invalid template name", str(ctx.exception))

    def test_delete_templates_directory_not_found(self):
        """Should raise FileNotFoundError when templates directory doesn't exist."""
        # Remove the templates directory
        shutil.rmtree(CONF.TEMPLATES_DIR)

        with self.assertRaises(FileNotFoundError) as ctx:
            TemplateManager.delete("anytemplate")
        self.assertIn("Templates directory not found", str(ctx.exception))

    def test_delete_default_template_removes_file_only(self):
        """Should delete the file but default template remains loadable from DEFAULT_TEMPLATES."""
        # Create a file for a default template
        template_path = CONF.TEMPLATES_DIR / "architect.txt"
        template_path.write_text("custom content", encoding="utf-8")
        self.assertTrue(template_path.exists())

        TemplateManager.delete("architect.txt")

        # File should be deleted
        self.assertFalse(template_path.exists())
        # But template should still be loadable from DEFAULT_TEMPLATES
        loaded = TemplateManager.load("architect.txt")
        self.assertIn("Senior Software Architect", loaded)

    def test_delete_strips_whitespace_from_name(self):
        """Should strip whitespace from template name before processing."""
        template_path = CONF.TEMPLATES_DIR / "mytemplate.txt"
        template_path.write_text("test content", encoding="utf-8")
