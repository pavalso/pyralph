...
class Config:
    ...
    def ensure_directories(self) -> None:
        """Create required directories for Ralph operation if they don't exist."""
...
class Logger:
    ...
    def set_no_color(enabled: bool) -> None:
        ...
        Args:
            enabled: If True, disables color codes in console output
        """
    ...
    def info(msg: str, color: str = "RESET") -> None:
        ...
        Args:
            ...
            color: Color name for the message (e.g., "GREEN", "RED", "CYAN")
        """
        if Logger.no_color:
            print(msg)
        else:
    ...
    def file_log(content: str, type: str, tag: str = "UNKNOWN") -> None:
        ...
        Args:
            ...
            type: Entry type (e.g., "PROMPT", "RESPONSE", "ERROR", "INFO")
            tag: Identifier tag for the log entry
        """
...
class MemoryManager:
    ...
    def get_structure() -> str:
        """
        Get a formatted list of all memory files.

        Returns:
            Newline-separated list of memory file paths, or "(Memory Empty)" if none exist
        """
    ...
    def extract_test_command() -> str:
        """
        Extract the test command from memory files.

        Searches memory files for a pattern like "Test Command: `pytest`".
        Falls back to "npm test" if package.json exists, otherwise "pytest".

        Returns:
            The test command string to execute
        """
        pattern = re.compile(r"Test Command.*?`([^`]+)`", re.IGNORECASE)
        for path in CONF.MEMORY_DIR.rglob('*'):
